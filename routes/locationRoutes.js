...
const DUPLICATE_DISTANCE_METERS = 15; // ✅ INCREASED TO 15 METERS
...
async function processLocationPoint({ userId, sessionId, latitude, longitude, accuracy, timestamp }) {
  ...
  // ✅ FIX: Check session status with better handling
  if (session.status !== 'ACTIVE') {
    console.warn(`⚠️ Session ${sessionId} is not active (status: ${session.status})`);
    
    // ✅ FIX: Try to reactivate if session is AUTO_ENDED from today
    if (session.status === 'AUTO_ENDED') {
      const startOfDay = new Date();
      startOfDay.setHours(0, 0, 0, 0);
      
      if (session.startTime >= startOfDay) {
        console.log(`🔄 Reactivating AUTO_ENDED session ${sessionId}`);
        ...
        return { 
          status: 400, 
          body: { 
            message: 'Session is from previous day',
      ...
      // ✅ FIX: Return detailed error for debugging
      return { 
        status: 400, 
        body: { 
  ...
  // ✅ Deduplication check with 15 meter threshold
