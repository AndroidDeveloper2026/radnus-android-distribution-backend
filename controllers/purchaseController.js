...
exports.getProductPriceHistory = async (req, res) => {
  try {
    ...
    // 1. Get product information with current prices
    const product = await Product.findById(productObjectId);
    if (!product) {
      return res.status(404).json({ msg: "Product not found" });
    }
    ...
    const purchases = await PurchaseEntry.aggregate([
      ...
      // Project required fields - use CURRENT product prices, NOT historical
      {
        $project: {
          purchaseNumber: 1,
          invoiceNumber: 1,
    ...
    // 3. Add CURRENT product prices to each history entry
    const historyWithCurrentPrices = purchases.map(p => ({
      ...p,
      // Use current product prices from the product document
      mrp: product.mrp || 0,
      distributorPrice: product.distributorPrice || 0,
      retailerPrice: product.retailerPrice || 0,
      walkinPrice: product.walkinPrice || 0,
    ...
    // 4. Calculate summary statistics using historical purchase prices only
    const prices = historyWithCurrentPrices.map(h => h.purchasePrice).filter(p => p !== null && p !== undefined);
    ...
    const summary = {
      ...
      averagePrice: prices.length > 0 
        ...
        : 0,
      totalPurchases: historyWithCurrentPrices.length,
      lastPurchaseDate: historyWithCurrentPrices.length > 0 ? historyWithCurrentPrices[0].purchaseDate : null
    };

    // 5. Prepare product info with current prices
    const productInfo = {
      _id: product._id,
      name: product.name,
      sku: product.sku,
    ...
    res.json({
      product: productInfo,
      summary: summary,
      history: historyWithCurrentPrices
    });
