...
exports.register = async (req, res) => {
  try {
    ...
    // NOTE: Mobile-number verification is no longer done via an
    // app-generated OTP pushed over FCM (that only proved control of
    // the *device*, not the entered mobile number). It is now done via
    // Firebase Phone Authentication on the client, which sends the SMS
    // directly to the number the user typed in. This backend only
    // records the account as unverified until /verify-otp confirms a
    // valid Firebase phone-auth ID token for this exact mobile number.
    await user.save();
...
// Normalize a phone number down to its last 10 digits so values like
// "9876543210", "+919876543210" and "919876543210" all compare equal.
function last10Digits(value) {
  return (value || "").toString().replace(/\D/g, "").slice(-10);
}

// VERIFY OTP (mobile ownership, via Firebase Phone Authentication)
//
// The client completes Firebase Phone Auth (which sends a real SMS to
// the number the user typed in) and obtains a Firebase ID token proving
// they control that phone number. We verify that token server-side with
// the Firebase Admin SDK and cross-check the phone number it attests to
// against the mobile number on the account being verified. This is what
// ties verification to the *entered mobile number* instead of merely
// the device (which the old FCM-based flow did not do).
exports.verifyOtp = async (req, res) => {
  try {
    let { mobile, idToken } = req.body;

    if (!mobile || !idToken) {
      return res.status(400).json({
        success: false,
        message: "Mobile number and verification token are required",
      });
    }
    ...
    // The token must have been issued via phone sign-in (i.e. it carries
    // a verified phone_number claim), and that number must match the
    // mobile number on THIS account — not just any verified number.
    if (
      !decoded.phone_number ||
      last10Digits(decoded.phone_number) !== last10Digits(mobile)
    ) {
      return res.status(400).json({
        success: false,
        message: "Verified phone number does not match the registered mobile number",
      });
    }

    user.isVerified = true;
    // Legacy fields from the old OTP flow — cleared out if still present.
    user.otp = null;
    user.otpExpiry = null;
...
exports.resendOtp = async (req, res) => {
  try {
    ...
    // REGISTER FLOW
    // Mobile-number OTPs for registration are now sent by Firebase Phone
    // Auth directly from the client (a real SMS to the entered number),
    // so there is nothing for the backend to resend here. This branch is
    // kept only to respond clearly if an old client build still calls it.
    if (type === "register") {
      return res.status(410).json({
        success: false,
        message:
          "Registration OTPs are now sent via SMS through the app directly. Please use the in-app resend option on the verification screen.",
      });
    }
...
//------------ 10.09.26 working FCM otp issue only -------
// // controllers/authController.js
// const bcrypt = require("bcrypt");
// const jwt = require("jsonwebtoken");
// const crypto = require("crypto");
