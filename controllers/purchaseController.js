...
// ─── NEW: Bulk batch availability for many products in ONE request ────────
// The OrderCartPage used to call GET /product-batches/:productId/availability
// once PER product in the catalog (an N+1 network problem — a few hundred
// products meant a few hundred requests, which is why the page took a very
// long time to load). This endpoint returns availability for every
// requested product in a single round trip.
exports.getBatchAvailabilityBulk = async (req, res) => {
  try {
    ...
    const batches = await StockBatch.find({
      ...
      .sort({ inwardDate: 1 })
      .select('productId batchNo quantityAvailable purchasePrice mrp retailerPrice distributorPrice walkinPrice inwardDate');

    const result = {};
    validIds.forEach(id => { result[id] = []; });
    ...
    console.error('getBatchAvailabilityBulk error:', err);
    res.status(500).json({ msg: err.message || "Failed to fetch batch availability" });
  }
};
