...
async function getBatchQueue(productId, session = null) {
  ...
  return batches.map((b, idx) => ({
    ...
    // ✅ FIX: Include ALL price fields from StockBatch
    mrp: b.mrp ?? 0,
...
async function consumeExplicitAllocations(productId, explicitAllocations, session) {
  ...
  for (const alloc of explicitAllocations) {
    ...
    if (!alloc.batchNumber) {
      throw new Error("Each explicit batch allocation must include a batchNumber");
    }

    const batch = await StockBatch.findOne({ productId, batchNo: alloc.batchNumber }).session(
      session || null
    );
    if (!batch) {
    ...
    const result = await StockBatch.updateOne(
      { _id: batch._id, quantityAvailable: { $gte: qty } },
      { $inc: { quantityAvailable: -qty } },
      { session }
    );
    if (result.matchedCount === 0) {
      throw new InsufficientStockError(productId, qty, batch.quantityAvailable);
    }
